...
│   ├── age.py           # 年龄自动识别：分层候选 + 安全关 + 取舍
│   ├── cn_number.py     # 中文数字转换
│   ├── simulation.py    # 人生模拟引擎：画像 → 逐年人生轨迹
│   └── day.py           # 逐日模拟引擎：一次一天，带上下文记忆
...
年度模拟给的是人生骨架（哪年升职、哪年结婚），逐日模拟给的是血肉：
**一次生成一整天**，从清晨到深夜六个时段、十来条具体的事，看完再决定
要不要继续下一天。

上下文记忆是这个模式的核心，靠四样东西实现：

| 机制 | 作用 |
|------|------|
| **状态连续** | 精力/心情/压力/健康/钱逐日结转。昨晚熬到两点，今早就起不来；连着加班，身体会出问题 |
| **剧情线** | 跨天的故事：赶不完的项目、刚萌芽的心动、一场感冒、坚持不下去的健身计划。每条线有阶段，自己开始也自己结束 |
| **人物记忆** | 出现过的人会被记住并反复出现；很久没见的人会在某个夜里被想起 |
| **近期记忆** | 今天的叙述会回指昨天（"昨晚睡得太晚，闹钟响了三遍你才起来"） |

状态向**人格决定的基线**回归而不是回归到零——高神经质的人即使今天顺利，
压力也不会真的归零，这正是"内耗"的含义。同一件好事，外向者心情涨得更多；
同一件坏事，神经质者受影响更大。

同一段经历重放会得到同一串日子（引擎内确定性）。
